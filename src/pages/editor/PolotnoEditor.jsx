...
const PolotnoEditor = observer(({store}) => {
    ...
    useEffect(() => {
        if (!id) return;
        if (path.pathname.includes('draft')) {
            axiosInstance.get(`/api/user-template/${id}`)
                .then((res) => {
                    const templateData = res?.data?.data;

                    const editorJson = templateData?.edited_object.designJson;
                    console.log('draft : ', editorJson)
